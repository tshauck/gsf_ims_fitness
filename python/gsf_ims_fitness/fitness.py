    ...
    if growth_plate_layout_file is None:
        ...
        if (len(inducer_list) == 1) and (len(tet_conc_list) == 2):
            ...
            inducer_conc_list_in_plate = (
                np.asarray(np.split(np.asarray(inducer_conc_list), 4))
                .transpose()
                .flatten()
                .tolist()
                * 8
            )
            inducer_conc_list_in_plate = np.asarray(
                [(inducer_conc_list[j::4] * 4) * 2 for j in range(4)] * 1
            ).flatten()

            ligand_list = [
                inducer if x > 0 else "none" for x in inducer_conc_list_in_plate
            ]
            ...
            ligand_list = [
                inducer if x > 0 else inducer_2 if y > 0 else "none"
                for x, y in zip(
                    inducer_conc_list_in_plate, inducer_2_conc_list_in_plate
                )
            ]
        ...
        if len(df) > 0:
            ...
            if "mmol/L" in np.unique(df.inducerUnits):
                if "umol/L" in np.unique(df.inducerUnits):
                    raise ValueError(
                        f"Growth plate file contains multiple inducer units (inducerUnits), {growth_plate_layout_file}"
                    )
        ...
        if len(antibiotic) == 1:
            ...
            raise ValueError(
                f"Growth plate file contains more then one antibiotic/selector, {growth_plate_layout_file}"
            )
        elif len(antibiotic) < 1:
            raise ValueError(
                f"Growth plate file does not contain an antibiotic/selector, {growth_plate_layout_file}"
            )
...
def total_plate_2_and_plot_bar_seq_quality(
    ...
    barcode_frame["fraction_total"] = (
        barcode_frame["total_counts"] / barcode_frame["total_counts"].sum()
    )

    plot_colors = sns.hls_palette(12, l=0.4, s=0.8)
    ...
    for i in range(13):
        axs[0].plot(
            [i * 8 + 0.5, i * 8 + 0.5], [min(BC_totals), max(BC_totals)], color="gray"
        )
    ...
    for ax in axs.flatten()[:2]:
        ax.plot([0, 0.125], [0, 0.125], color="k")
    for ax in axs.flatten()[2:4]:
        ax.plot([0, 0.125], [0, 0], color="k")
    ...
    fig.suptitle(
        "Fraction from Each Dual Barcode (Plate 2)", fontsize=24, position=(0.5, 0.905)
    )
...
def plot_bar_seq_read_fractions(
    barcode_frame,
    notebook_dir,
    experiment=None,
    show_plots=True,
...
def fit_barcode_fitness(
    barcode_frame, notebook_dir, experiment=None, inducer_conc_list=None, max_fits=None
):
    ...
    spike_in_fitness_0 = {
        "AO-B": np.array([0.9637] * 12),
        "AO-E": np.array([0.9666] * 12),
    }
    spike_in_fitness_tet = {
        "AO-B": np.array([0.8972] * 12),
        "AO-E": np.array([0.8757] * 12),
    }

    spike_in_row = {
        "AO-B": barcode_frame[ref_index_b : ref_index_b + 1],
        "AO-E": barcode_frame[ref_index_e : ref_index_e + 1],
    }
    ...
    for spike_in, initial in zip(["AO-B", "AO-E"], ["b", "e"]):
        ...
        spike_in_reads_0 = [
            spike_in_row[spike_in][f"read_count_0_{plate_num}"].values[0]
            for plate_num in range(2, 6)
        ]
        spike_in_reads_tet = [
            spike_in_row[spike_in][f"read_count_tet_{plate_num}"].values[0]
            for plate_num in range(2, 6)
        ]
            ...
            for j in range(len(n_reads[0])):  # iteration over IPTG concentrations 0-11
                ...
                for i in range(len(n_reads)):  # iteration over time points 0-3
                    if n_reads[i][j] > 0 and spike_in_reads_0[i][j] > 0:
                        ...
                        s.append(
                            np.sqrt(1 / n_reads[i][j] + 1 / spike_in_reads_0[i][j])
                        )
                if len(x) > 1:
                    popt, pcov = curve_fit(
                        line_funct, x, y, sigma=s, absolute_sigma=True
                    )
            ...
            for j in range(len(n_reads[0])):  # iteration over IPTG concentrations 0-11
                ...
                for i in range(len(n_reads)):  # iteration over time points 0-3
                    if n_reads[i][j] > 0 and spike_in_reads_tet[i][j] > 0:
                        x.append(x0[i])
                        y.append(
                            np.log(n_reads[i][j]) - np.log(spike_in_reads_tet[i][j])
                        )
                        s.append(
                            np.sqrt(1 / n_reads[i][j] + 1 / spike_in_reads_tet[i][j])
                        )
                if len(x) > 1:

                    def fit_funct(xp, mp, bp):
                        return bi_linear_funct(xp - 2, mp, bp, slope_0, alpha=np.log(5))
...
def plot_barcode_fitness(
    ...
    for (index, row), ax in zip(barcode_frame.iterrows(), axs):  # iterate over barcodes
        for initial in ["b", "e"]:
            ...
            ax.errorbar(
                x, y, s, marker="o", ms=10, color=plot_colors[0], fillstyle=fill_style
            )
            y = row[f"fitness_tet_estimate_{initial}"]
            s = row[f"fitness_tet_err_{initial}"]
            ax.errorbar(
                x, y, s, marker="^", ms=10, color=plot_colors[1], fillstyle=fill_style
            )
...
def bi_linear_base(x, m2, m1, alpha, x0):
    return (
        m2 * x
        - (m1 * np.log(1 + np.exp(alpha * (-x + x0)))) / alpha
        + (m2 * np.log(1 + np.exp(alpha * (-x + x0)))) / alpha
    )


def bi_linear_funct_2(z, m2, b, m1, alpha, x0):
    return (
        b + bi_linear_base(z, m2, m1, alpha, x0) - bi_linear_base(0, m2, m1, alpha, x0)
    )
    ...
    if plasmid == "pVER":
        ...
        if use_geo_mean:
            if tet_conc == 20:
                ...
                params = np.array(
                    [-0.7473, 4.538e03, 1.849, 0.008271, 93.44, 0.03828]
                ...
                [-0.7473, 4.538e03, 1.849]
            elif tet_conc == 1.25:
                params = np.array(
                    [-0.7949, 239.3, 0.9777, 0.05126, 35.01, 0.04737]
                )  # from fit to 8 RSs and several other numbered variants using 4-lane barseq data from 2022-11-08_two-lig_two-sel_DNA-5-plates
        else:
            if tet_conc == 10:
                # from fit to 8 RSs and several other numbered variants using 4-lane barseq data from 2022-11-08_two-lig_two-sel_DNA-5-plates
                params = np.array([-0.7445, 4.967e03, 1.891, 0.008442, 104.2, 0.04059])
                params = np.array([-0.7400, 4.983e03, 1.871, 0.00597, 71.95, 0.0277])
                params = np.array([-0.7222, 4.76e03, 1.762, 0.00897, 111.9, 0.04116])
            elif tet_conc == 1.25:
                # from fit to 8 RSs and several other numbered variants using 4-lane barseq data from 2022-11-08_two-lig_two-sel_DNA-5-plates
                ...
                params = np.array(
                    [-0.72246, 13328, 3.2374]
                )  # place-holder values, for testing
    elif plasmid == "pRamR":
        # For RamR, params are: high_fitness, mid_g, fitness_n, high_fitness_err, mid_g_err, fitness_n_err,
        params = [-1.604, 1.017e03, 1.495, 0.02018, 44.55, 0.06434]
    else:
        params = np.array([-7.41526290e-01, 7.75447318e02, 2.78019804e00])
