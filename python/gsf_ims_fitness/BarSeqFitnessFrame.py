...
class BarSeqFitnessFrame:
    def __init__(
        ...
        f_cluster_file = list(self.data_directory.glob("*forward_merged_cluster.csv"))[
            0
        ]
        r_cluster_file = list(self.data_directory.glob("*reverse_merged_cluster.csv"))[
            0
        ]
        ...
        for_barcode_clusterID_dict = dict(
            zip(f_bc_cluster_frame["Center"], f_bc_cluster_frame["Cluster.ID"])
        )
        rev_barcode_clusterID_dict = dict(
            zip(r_bc_cluster_frame["Center"], r_bc_cluster_frame["Cluster.ID"])
        )

        for_barcode_score_dict = dict(
            zip(f_bc_cluster_frame["Center"], f_bc_cluster_frame["Cluster.Score"])
        )
        rev_barcode_score_dict = dict(
            zip(r_bc_cluster_frame["Center"], r_bc_cluster_frame["Cluster.Score"])
        )

        barcode_frame["for_BC_ID"] = [
            for_barcode_clusterID_dict[x] for x in barcode_frame["forward_BC"]
        ]
        barcode_frame["rev_BC_ID"] = [
            rev_barcode_clusterID_dict[x] for x in barcode_frame["reverse_BC"]
        ]

        barcode_frame["for_BC_Score"] = [
            for_barcode_score_dict[x] for x in barcode_frame["forward_BC"]
        ]
        barcode_frame["rev_BC_Score"] = [
            rev_barcode_score_dict[x] for x in barcode_frame["reverse_BC"]
        ]
        ...
        if single_barcode:
            ...
            with open(merge_log_file, "w") as log_file:
                ...
                for_barcode_count_dict = dict(
                    zip(
                        f_bc_cluster_frame["Cluster.ID"],
                        f_bc_cluster_frame["time_point_1"],
                    )
                )
                rev_barcode_count_dict = dict(
                    zip(
                        r_bc_cluster_frame["Cluster.ID"],
                        r_bc_cluster_frame["time_point_1"],
                    )
                )
                    ...
                    for bc, y in zip(bc_list[1:], x[1:]):
                        if len(bc) == len(bc_list[0]):
                            ...
                            if dist > merge_dist_cutoff:
                                log_file.write(
                                    f"    Warning, forward barcodes NOT merged with Hamming distance > cutoff, {dist}:\n"
                                )
                                log_file.write(
                                    f"        {bc_list[0]}, ID: {x[0]}, count: {for_barcode_count_dict[x[0]]}\n"
                                )
                                log_file.write(
                                    f"        {bc}, ID: {y}, count: {for_barcode_count_dict[y]}\n"
                                )
                            else:
                                bc_merge_dict[y] = x[0]
                                if dist > 1:
                                    log_file.write(
                                        f"    Warning, merging forward barcodes with Hamming distance {dist}:\n"
                                    )
                                else:
                                    log_file.write(
                                        f"    Merging forward barcodes with Hamming distance {dist}:\n"
                                    )
                                log_file.write(
                                    f"        {bc_list[0]}, ID: {x[0]}, count: {for_barcode_count_dict[x[0]]}\n"
                                )
                            ...
                            if dist - len_diff > merge_dist_cutoff:
                                log_file.write(
                                    f"    Warning, forward barcodes NOT merged with Levenshtein distance > cutoff, {dist}:\n"
                                )
                                log_file.write(
                                    f"        {bc_list[0]}, ID: {x[0]}, count: {for_barcode_count_dict[x[0]]}\n"
                                )
                                ...
                                if dist > 1:
                                    log_file.write(
                                        f"    Warning, merging forward barcodes with Levenshtein distance {dist}:\n"
                                    )
                                else:
                                    log_file.write(
                                        f"    Merging forward barcodes with Levenshtein distance {dist}:\n"
                                    )
                                log_file.write(
                                    f"        {bc_list[0]}, ID: {x[0]}, count: {for_barcode_count_dict[x[0]]}\n"
                                )
                ...
                new_bc_id_list = [
                    bc_merge_dict[x] if x in bc_merge_dict else x
                    for x in barcode_frame.for_BC_ID
                ]
    ...
    def trim_and_sum_barcodes(
        ...
        with warnings.catch_warnings():
            ...
            barcode_frame["fraction_total"] = barcode_frame["total_counts"] / (
        ...
        barcode_frame["fraction_total_p2"] = barcode_frame["total_counts_plate_2"] / (
        ...
        if ref_seq_frame is not None:
            ...
            for _, row in ref_seq_frame.iterrows():
                ...
                if len(display_frame) == 0:
                    ...
                    barcode_frame.loc[display_frame.index[0], "RS_name"] = row[
                        "RS_name"
                    ]

                    if self.plasmid == "Align-TF":
                        display_frame["transcription_factor"].iloc[0] = row[
                            "transcription_factor"
                        ]
                        barcode_frame.loc[
                            display_frame.index[0], "transcription_factor"
    ...
    def mark_chimera_parents(self):
        ...
        for index, row in barcode_frame[barcode_frame["possibleChimera"]].iterrows():
            ...
            if (len(for_matches) > 0) & (len(rev_matches) > 0):
                ...
                geo_mean_p2 = np.sqrt(
                    rev_matches["total_counts_plate_2"].values[0]
                    * for_matches["total_counts_plate_2"].values[0]
                )
    ...
    def flag_possible_chimeras(self, use_faster_search=True, faster_search_ratio=10):
        ...
        for index, row in barcode_frame[1:].iterrows():
            if use_faster_search:
                ...
                comp_data = barcode_frame[
                    barcode_frame["total_counts"] > faster_search_ratio * data_counts
                ]
            else:
            ...
            if (len(for_matches) > 0) & (len(rev_matches) > 0):
                ...
                barcode_frame.at[index, "reverse_parent"] = rev_matches.index[0]
                geo_mean = np.sqrt(
                    rev_matches["total_counts"].values[0]
                    * for_matches["total_counts"].values[0]
                )
    ...
    def stan_barcode_slope(
        ...
        if index is None:
            ...
            for samp in fitness_list_dict.keys():
                fit_frame[f"fit_slope_S{samp}_{stan_str}{initial}"] = fitness_list_dict[
                    samp
                ]
                fit_frame[f"fit_slope_S{samp}_err_{stan_str}{initial}"] = err_list_dict[
                    samp
                ]
                fit_frame[f"fit_slope_S{samp}_resid_{stan_str}{initial}"] = list(
                    resid_list_dict[samp]
                )
                fit_frame[f"fit_slope_S{samp}_log_ratio_out_{stan_str}{initial}"] = (
                    list(log_ratio_q_dict[samp])
                )
    ...
    def display_viewable_plate_layouts(self):
        ...
        plate_layout_frame_2 = pd.DataFrame(
            {r: cont for r, cont in zip(fitness.rows(), col_contents)},
        ...
        gp_samps_arr = np.array([[n for n in range(1, 13)], [n for n in range(13, 25)]])
        ...
        plate_layout_frame_3.rename(
            columns={n: n + 1 for n in plate_layout_frame_3.columns}, inplace=True
        )
    ...
    def set_sample_plate_map(
        ...
        if existing_plate_map is not None:
            ...
            if np.any(df_diff.values):
                raise Exception(
                    "The set_sample_plate_map method is attempting to change the previously defined sample_plate_map. If this is what you want to do, manually set sample_plate_map to None, then re-run set_sample_plate_map"
                )
        ...
        for samp_list in [ref_samples, non_ref_without_tet]:
            for samp in samp_list:
                ...
                if use_all_samples_model:
                    ...
                    stan_fit = stan_model_no_tet.sample(
                        data=stan_data,
                        iter_sampling=iter_sampling,
                        iter_warmup=iter_warmup,
                        chains=chains,
                    ...
                    log_ratio_out_quantiles = np.quantile(
                        fit_result, [0.05, 0.25, 0.5, 0.75, 0.95], axis=0
                    )

                    fitness_out_dict[samp] = [
                        fit_mu,
                        fit_sig,
                        fit_resid,
                        log_ratio_out_quantiles,
                    ]
            ...
            if use_all_samples_model:
                ...
                if stan_fit != "failed":
                    ...
                    with warnings.catch_warnings():
                        ...
                        fit_resid = (
                            np.log(n_reads)
                            - np.log(spike_in_reads)
                            - np.mean(stan_fit.stan_variable("log_ratio_out"), axis=0)
                        )
                    log_ratio_out_quantiles = np.quantile(
                        stan_fit.stan_variable("log_ratio_out"),
        ...
        if use_all_samples_model:
            ...
            if not return_fits:
                ...
                for samp_list, samp_str in zip(sample_lists, sample_str_list):
                    ...
                    log_ratio_out_list = np.quantile(
                        fit_result, [0.05, 0.25, 0.5, 0.75, 0.95], axis=0
                    ).transpose([2, 0, 1])
                    for samp, mu, sig, res, log_rat in zip(
                        samp_list,
                        fit_mu_list,
                        fit_sig_list,
                        fit_resid_list,
    ...
    def plot_count_ratio_per_sample(
        self,
        plot_range=None,
        ...
        for ind, row in plot_frame.iterrows():
            ...
            if self.plasmid == "Align-TF" and ("norm" not in row.RS_name):
                ...
                plot_list = [s for s in plot_list_0 if s in samples_with_tf] + [
                    s for s in plot_list_0 if s not in samples_with_tf
                ]
            ...
            for samp, ax in zip(plot_list, axs):
                ...
                if plot_raw_counts:
                    ...
                    ax.errorbar(
                        x[n_reads > 0], y[n_reads > 0], s[n_reads > 0], fmt="o", ms=10
                    )
                else:
                    with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
                    ...
                    log_ratio = row[
                        f"fit_slope_S{samp}_log_ratio_out_{spike_in_initial}"
                    ]
                    if log_scale is not None:
    ...
    def plot_or_fit_barcode_ratios(
        ...
        for spike_in, initial in zip([spike_1, spike_2], [spike_1_init, spike_2_init]):
            ...
            for samp in samples_without_tet:
                ...
                resids_list = []
                log_ratio_out_list = []
                for (index, row), ax in zip(
                    fit_frame.iterrows(), axs
                    ...
                    with warnings.catch_warnings():
                        ...
                        y = np.log(n_reads) - np.log(spike_in_reads)
                        s = np.sqrt(
                            1 / n_reads + 1 / spike_in_reads + min_log_count_error**2
                        )
                    ...
                    if plots_not_fits:
                        ...
                        if (initial in show_spike_ins) and (samp in plot_samples):
                            ax.errorbar(
                                x, y, s, fmt="o", label=f"{samp}-{initial}", ms=10
                            )
                    else:
                        if len(x) > 1:
                            popt, pcov = curve_fit(
                ...
                if not plots_not_fits:
                    fit_frame[f"fit_slope_S{samp}_{early_initial}{initial}"] = (
                        f_est_list
                    )
                    fit_frame[f"fit_slope_S{samp}_err_{early_initial}{initial}"] = (
                        f_err_list
                    )
                    fit_frame[f"fit_slope_S{samp}_resid_{early_initial}{initial}"] = (
                        list(resids_list)
                    )
                    fit_frame[
                        f"fit_slope_S{samp}_log_ratio_out_{early_initial}{initial}"
                    ] = list(log_ratio_out_list)
                ...
                for (index, row), slope_0, ax in zip(
                    ...
                    if early_slope or mid_slope:
                        ...
                        if bi_linear_x0 is None:
                            ...
                            def fit_funct(xp, mp, bp):
                                return fitness.bi_linear_funct(
                                    xp - 2, mp, bp, slope_0, alpha=bi_linear_alpha
                                )
                        else:

                            def fit_funct(xp, mp, bp):
                                return fitness.bi_linear_funct_2(
                                    xp - 2,
                                    mp,
                                    bp,
                                    slope_0,
                    ...
                    if plots_not_fits:
                        if (initial in show_spike_ins) and (samp in plot_samples):
                            ax.errorbar(
                                x, y, s, fmt="^", label=f"{samp}-{initial}", ms=10
                            )
                    else:
                        if len(x) > 1:
                            try:
                                popt, pcov = curve_fit(
                                    fit_funct, x, y, sigma=s, absolute_sigma=True
                                )
                            except RuntimeError:
                                try:
                                    popt, pcov = curve_fit(
        ...
        for samp in sample_list:
            ...
            if constant_spike_in:
                ...
                if plasmid == "pVER":
                    ...
                    spike_in_fitness_err = spike_in_fitness_dict[tet_conc][spike_in](
                        ligand, lig_conc
                    )[1]
                elif plasmid == "pRamR":
                    lig_conc = max(df[self.ligand_list].iloc[0].values)
                    spike_in_fitness = spike_in_fitness_dict[tet_conc][spike_in][0](
                        lig_conc
                    )
                    spike_in_fitness_err = spike_in_fitness_dict[tet_conc][spike_in][1](
                        lig_conc
                    )
                elif plasmid == "pCymR":
                    ligand = df.ligand.iloc[0]
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                fit_frame[f"fitness_S{samp}_{initial}"] = spike_in_fitness + fit_frame[
                    f"fit_slope_S{samp}_{initial}"
                ] / np.log(10)
    ...
    def plot_fit_residuals(self, initial=None, alpha=0.3):
        ...
        for samp, mean_list, rms_list, marker, tet in zip(
            sample_list, mean_resid_lists, rms_resid_lists, marker_list, tet_list
        ):
    ...
    def set_fit_fitness_difference_params(
        ...
        if fit_fitness_difference_params is not None:
            ...
            fit_fitness_difference_params = [
                fitness.fit_fitness_difference_params(plasmid=plasmid, tet_conc=x)
    ...
    def stan_fitness_difference_curves(
        ...
        def stan_fit_row(st_row, st_index, lig_list, return_fit=False):
            ...
            try:
                stan_data = self.bs_frame_stan_data(
                    st_row, initial=initial, is_gp_model=is_gp_model
                )
                ...
                stan_samples_arr = np.array(
                    [stan_fit.stan_variable(key) for key in params_list]
                )
                ...
                stan_samples_out = rng.choice(
                    stan_quant_arr, size=32, replace=False, axis=1, shuffle=False
                )
                stan_quantiles = np.array(
                    [np.quantile(x, quantile_list) for x in stan_quant_arr]
                )
                ...
                hill_on_at_zero_prob = len(
                    g0_samples[g0_samples > wild_type_ginf / 4]
                ) / len(g0_samples)
                if len(lig_list) == 1:
                    g_ratio_samples = stan_samples_arr[log_ginf_g0_ind]
                    hill_invert_prob = len(g_ratio_samples[g_ratio_samples < 0]) / len(
                        g_ratio_samples
                    )
                elif len(ligand_list) == 2:
                    g_ratio_samples = [
                        stan_samples_arr[k]
                        for k in [log_ginf_g0_ind_1, log_ginf_g0_ind_2]
                    ]
        ...
        if refit_indexes is None:
            ...
            fit_list = [
                stan_fit_row(row, index, ligand_list)
                for (index, row) in zip(refit_indexes, row_list)
            ]
        ...
        if refit_indexes is None:
            ...
            for i, lig in enumerate(ligand_list):
                barcode_frame[f"hill_invert_prob_{lig}"] = np.array(
                    invert_prob_list
                ).transpose()[i]
            ...
            for i, lig in enumerate(ligand_list):
                barcode_frame.loc[index_list, f"hill_invert_prob_{lig}"] = np.array(
                    invert_prob_list
                ).transpose()[i]
    ...
    def stan_single_fitness_to_function(
        self,
        adapt_delta=0.9,
        iter_warmup=500,
        iter_sampling=500,
        ...
        if plasmid != "Align-TF":
            raise NotImplementedError(
                f"stan_single_fitness_to_function() is not yet implemented for plasmid: {plasmid}"
            )
        ...
        for c in antibiotic_conc_list:
            if c not in self.antibiotic_conc_list:
                raise ValueError(
                    f"Antibiotic concentrations included as first element of initial_min_err_list tuples must be in experiment antibiotic_conc_list: {antibiotic_conc_list}."
                )
        ...
        def stan_fit_row(st_row, return_fit=False):
            ...
            if stan_data is None:
                ...
                stan_fitness_params_mean = [
                    np.full((n_anti), np.nan) for p in fitness_params_list
                ]
                stan_fitness_params_std = [
                    np.full((n_anti), np.nan) for p in fitness_params_list
                ]
                ...
                if st_lig != ligand:
                    raise ValueError(
                        f"Stan data ligand_id ({st_lig}) does not match exprected value ({ligand}) for index {st_index}"
                    )
                ...
                try:
                    ...
                    stan_fitness_params_mean = [
                        np.mean(stan_fit.stan_variable(p)) for p in fitness_params_list
                    ]
                    stan_fitness_params_std = [
                        np.std(stan_fit.stan_variable(p)) for p in fitness_params_list
                    ]
                    ...
                    stan_fitness_params_mean = [
                        np.full((fit_data["N_antibiotic"]), np.nan)
                        for p in fitness_params_list
                    ]
                    stan_fitness_params_std = [
                        np.full((fit_data["N_antibiotic"]), np.nan)
                        for p in fitness_params_list
        ...
        for tf, lig in zip(tf_list, ligand_list):
            ...
            if len(conc) == 1:
                ...
                raise ValueError(
                    f"Unexpected number of ligand concentrations for transcription_factor {tf} and ligand {lig}"
                )

        output_columns = ["log_g0"]
        output_columns += [
            f"log_g_{non_zero_lig_conc_dict[lig]}_{lig}" for lig in ligand_list
        ]
        ...
        for ret_result in fit_list:  # iterate over fit results for each barcode/row
            ...
            for c, log_g_mu, lig_g_sig in zip(
                ret_result["ligand_concentrations"],
            ...
            for lig in ligand_list:
                if lig != ligand:
                    ...
                    output_dict[f"log_g_{non_zero_lig_conc_dict[lig]}_{lig}_err"] = (
                        np.nan
                    )
        ...
        if refit_indexes is None:
            ...
            for column_name in output_columns:
                barcode_frame[column_name] = [
                    x[column_name] for ind, x in enumerate(output_results_list)
                ]
            ...
            for column_name in output_columns:
                barcode_frame.loc[index_list, column_name] = [
                    x[column_name] for x in output_results_list
                ]
    ...
    def stan_GP_curves(
        ...
        key_params = (
            params_list + g_arr_list + g_ratio_arr_list + dg_arr_list + f_arr_list
        )
        ...
        def stan_fit_row(st_row, st_index, lig_list, return_fit=False):
            ...
            try:
                ...
                ret_dict["stan_g"] = [
                    np.array([np.quantile(a, q, axis=0) for q in quantile_list])
                    for a in g_arr
                ]
                ret_dict["stan_g_ratio"] = np.array(
                    [np.mean(a, axis=0) for a in g_ratio_arr]
                )
                ret_dict["stan_g_ratio_err"] = np.array(
                    [np.std(a, axis=0) for a in g_ratio_arr]
                )
                ret_dict["stan_dg"] = [
                    np.array([np.quantile(a, q, axis=0) for q in quantile_list])
                    for a in dg_arr
                ...
                ret_dict["stan_g_samples"] = [
                    rng.choice(
                        a, size=32, replace=False, axis=0, shuffle=False
                    ).transpose()
                    for a in g_arr
                ]
                ret_dict["stan_dg_samples"] = [
                    rng.choice(
                        a, size=32, replace=False, axis=0, shuffle=False
                    ).transpose()
                    for a in dg_arr
                ...
                ret_dict["stan_g"] = [
                    np.full((quantile_dim, x_dim), np.nan)
                    for i in range(len(g_arr_list))
                ]
                ret_dict["stan_g_ratio"] = np.array(
                    [np.full(x_dim, np.nan) for i in range(len(g_arr_list))]
                )
                ret_dict["stan_g_ratio_err"] = np.array(
                    [np.full(x_dim, np.nan) for i in range(len(g_arr_list))]
                )
                ret_dict["stan_dg"] = [
                    np.full((quantile_dim, x_dim), np.nan)
                    for i in range(len(dg_arr_list))
                ]
                ret_dict["stan_f"] = [
                    np.full((quantile_dim, x_dim), np.nan)
                    for i in range(len(f_arr_list))
                ]

                ret_dict["stan_g_var"] = [
                    np.full(x_dim, np.nan) for i in range(len(g_arr_list))
                ]
                ret_dict["stan_dg_var"] = [
                    np.full(x_dim, np.nan) for i in range(len(dg_arr_list))
                ]

                ret_dict["stan_g_samples"] = [
                    np.full((x_dim, 32), np.nan) for i in range(len(g_arr_list))
                ]
                ret_dict["stan_dg_samples"] = [
                    np.full((x_dim, 32), np.nan) for i in range(len(dg_arr_list))
                ]
        ...
        if refit_indexes is None:
            ...
            for g_list, param, g_var, g_samp in zip(
                stan_g_list, g_arr_list, stan_g_var_list, stan_g_samples_list
            ):
                col_name = param
            ...
            for i, lig in enumerate(ligand_list):
                ...
                barcode_frame[col_name] = [
                    x[i] for x in dict_of_fit_lists["stan_g_ratio"]
                ]
                barcode_frame[f"{col_name}_err"] = [
                    x[i] for x in dict_of_fit_lists["stan_g_ratio_err"]
                ]

            for dg_list, param, dg_var, dg_samp in zip(
                stan_dg_list, dg_arr_list, stan_dg_var_list, stan_dg_samples_list
            ):
                col_name = param
                if col_name == "dlog_g":
                    col_name = "dlog_g_1"
            ...
            for ind, new_popt, new_pcov in zip(
                index_list,
                dict_of_fit_lists["stan_popt"],
                dict_of_fit_lists["stan_pcov"],
            ):
                barcode_frame.at[ind, "GP_params"] = new_popt
                barcode_frame.at[ind, "GP_cov"] = new_pcov

            for ind, rew_ratio, new_ratio_err in zip(
                index_list,
                dict_of_fit_lists["stan_g_ratio"],
                dict_of_fit_lists["stan_g_ratio_err"],
            ):
                for i, lig in enumerate(ligand_list):
                    col_name = f"GP_log_g_ratio_{lig}"
            ...
            for g_list, param, g_var, g_samp in zip(
                ...
                for i, lig in enumerate(ligand_list):
                    col_name = col_name.replace(f"_{i+1}", f"_{lig}")
                col_name = f"GP_{col_name}"
                for ind, new_g, new_var, new_samp in zip(
                    index_list, list(g_list), list(g_var), list(g_samp)
                ):
            ...
            for dg_list, param, dg_var, dg_samp in zip(
                ...
                for i, lig in enumerate(ligand_list):
                    col_name = col_name.replace(f"_{i+1}", f"_{lig}")
                col_name = f"GP_{col_name}"
                for ind, new_dg, new_dvar, new_dsamp in zip(
                    index_list, list(dg_list), list(dg_var), list(dg_samp)
                ):
    ...
    def plot_count_hist(
        ...
        if hist_bin_max is None:
            hist_bin_max = np.quantile(barcode_frame.total_counts, 0.99)
        ...
        axs[0].hist(
            barcode_frame["total_counts"], bins=bins, histtype="step", cumulative=-1
        )
    ...
    def plot_stdev(
        ...
        fig.suptitle(
            "Fraction from Each Dual Barcode (Plate 2)",
            fontsize=24,
            position=(0.5, 0.905),
        )
        ...
        for ax, y, lab in zip(
            axs, [per_well_log_mu, per_well_log_std], ["mean", "std"]
        ):
        ...
        x_test = np.asarray(
            [
                ...
                0.08814193520270863,
                0.07140559171457407,
                0.032268913991628186,
                0.02486533840744069,
                0.009370452839984682,
                0.0021539027931815613,
                0.0001936817014361814,
            ]
        )
        y_test = np.asarray(
            [
                0.0019726945744597706,
                0.0028398295224567756,
                0.0027140121666701543,
                0.0016422861817864806,
                0.0012364410886752844,
                0.0014467832918787287,
                0.0009412184378809117,
                0.0007090217957749182,
                0.00034552377974558844,
                0.00017198555940160456,
                4.958998052635534e-05,
            ]
        )
        poisson_err_test = np.asarray(
            [
                0.001391130466104952,
                0.001320415964490587,
                0.0011032026255463198,
                0.0009247685041703838,
                0.0008466282838575875,
                0.0007620910541483005,
                0.0005123905962175842,
                0.000449754496329767,
                0.00027605091052578906,
                0.0001323496187650663,
                3.929704870026295e-05,
            ]
        )
        #####################################################################################################
        ...
        x_small = np.asarray(
            [
                ...
                0.02536420185939611,
                0.03923343837910993,
                0.020238576239101202,
            ]
        )
        y_small = np.asarray(
            [
                0.003020200426682457,
                0.003374150359051314,
                0.00374541788260866,
                0.0035764736646941536,
                0.002598176841078495,
                0.003669639858790278,
                0.0021759993522437074,
                0.002827475646549457,
                0.0038335541520843315,
                0.002201298340428577,
                0.008012477386731139,
                0.001454772893578839,
                0.0012788004626381614,
        ...
        axs[0].plot(
            x_small, y_small, "o", ms=10, label="Small Library Selection, 2019-10-08"
        )
        axs[1].plot(
            x_test, y_test / x_test, "o", ms=10, label="Library Prep Test, 2019-10-02"
        )
        ...
        err_est = f_data["fraction_total_p2"] / np.sqrt(
            f_data["total_counts_plate_2"] / 24
        )
    ...
    def plot_fitness_curves(
        ...
        for (index, row), ax in zip(
            barcode_frame.iterrows(), axs
        ):  # iterate over barcodes
            if self.plasmid == "Align-TF":
                tf = row.transcription_factor
            for initial, fill_style, plot_st, plot_corr in zip(
                plot_initials,
                ["full", "none", "right", "left"],
                plot_stan_data,
                plot_w_ramr_correction,
            ):
                if old_style_plots:
                    ...
                    for tet, marker, antibiotic_color in zip(
                        antibiotic_conc_list, ["o", "<", ">", "^", "v"], fit_plot_colors
                    ):
                        for j, (lig, lig_color) in enumerate(
                            zip(ligand_list, fit_plot_colors)
                        ):
                            df = plot_df
                            ...
                            if plot_slope_not_fitness:
                                y = [
                                    row[f"fit_slope_S{i}_{initial}"] * fit_scale
                                    for i in df.sample_id
                                ]
                                s = [
                                    row[f"fit_slope_S{i}_err_{initial}"] * fit_scale
                                    for i in df.sample_id
                                ]
                            else:
                                if plot_st and (tet > 0):
                                    stan_data = self.bs_frame_stan_data(
                                        row,
                                        initial=initial,
                                        apply_ramr_correction=plot_corr,
                                    )
                                    if len(antibiotic_conc_list) == 2:
                                        ...
                                        if "y_0" in stan_data:
                                            ...
                                            y = (
                                                np.array(
                                                    st_y_0 + list(stan_data[f"y_{j+1}"])
                                                )
                                                + stan_data["y_ref"]
                                            )
                                            s = np.array(
                                                st_y_0_err
                                                + list(stan_data[f"y_{j+1}_err"])
                                            )
                                        else:
                                            x = stan_data["x"]
                                            y = stan_data["y"]
                                        ...
                                        y = (
                                            np.array(
                                                st_y_0
                                                + list(
                                                    stan_data[f"y_{j+1}_{tet_str}_tet"]
                                                )
                                            )
                                            + stan_data["y_ref"]
                                        )
                                        s = np.array(
                                            st_y_0_err
                                            + list(
                                                stan_data[f"y_{j+1}_{tet_str}_tet_err"]
                                            )
                                        )
                                else:
                                    y = [
                                        row[f"fitness_S{i}_{initial}"] * fit_scale
                                        for i in df.sample_id
                                    ]
                                    s = [
                                        row[f"fitness_S{i}_err_{initial}"] * fit_scale
                                        for i in df.sample_id
                                    ]
                            with warnings.catch_warnings():
                                warnings.simplefilter("ignore")
                ...
                if initial == plot_initials[0]:
                    ...
                    if plot_slope_not_fitness:
                        ax.set_ylabel(
                            f"Log slope relative to {plot_initials}", size=ax_label_size
                        )
                    else:
        ...
        if self.plasmid == "pVER":
            for initial in plot_initials:
                for i in range(1, 25):
                    ...
                    old_style_plots = old_style_plots or (
    ...
    def plot_dose_response(
        ...
        for lig, color in zip(ligand_list, fit_plot_colors):
            ...
            for i in range(1, 3):
                axg.fill_between(
                    x, stan_g[2 - i], stan_g[2 + i], alpha=fill_alpha, color=color
                )
            axg.legend(
                loc="upper left",
    ...
    def plot_fitness_and_difference_curves(
        self,
        save_plots=False,
        plot_range=None,
        include_ref_seqs=False,
        ...
        if plot_initials is None:
            if self.plasmid in ["pVER", "pCymR"]:
                ...
                plot_initials = [
                    self.get_default_initial(),
                    f"ea.{self.get_default_initial()}",
                ]
                ...
                if len(barcode_frame["sensor_params"].iloc[0]) == 7:
                    ...
                    def fit_funct(x, g_min, g_max, x_50, nx):
                        return hill_function_utils.double_hill_funct(
                            ...
                            x_50,
                            nx,
                            fit_fitness_difference_params[0],
                            0,
                            fit_fitness_difference_params[1],
                            fit_fitness_difference_params[2],
                        )
            else:
                if self.plasmid in ["pVER", "pCymR"]:
        ...
        for index, row in barcode_frame.iterrows():  # iterate over barcodes
            ...
            for initial in plot_initials:
                ...
                if old_style_plots:
                    for tet, color in zip(antibiotic_conc_list, fit_plot_colors):
                        y = (
                            row[f"fitness_{tet}_estimate_{initial}"]
                            * fitness_scale_factor
                        )
                        s = row[f"fitness_{tet}_err_{initial}"] * fitness_scale_factor
                        axl.errorbar(
                            x, y, s, marker="o", ms=8, color=color, fillstyle=fill_style
                        )

                    y_zero = row[f"fitness_{0}_estimate_{initial}"]
                    s_zero = row[f"fitness_{0}_err_{initial}"]
                    y_high = row[
                        f"fitness_{antibiotic_conc_list[1]}_estimate_{initial}"
                    ]
                    ...
                    for tet, marker in zip(antibiotic_conc_list, ["o", "<", ">"]):
                        for lig, color in zip(ligand_list, fit_plot_colors):
                            ...
                            s = (
                                np.array(
                                    [
                                        row[f"fitness_S{i}_err_{initial}"]
                                        for i in df.sample_id
                                    ]
                                )
                                * fitness_scale_factor
                    ...
                    if "ea" not in initial:
                        for tet, marker in zip(antibiotic_conc_list, ["o", "<", ">"]):
                            ...
                            if tet > 0:
                                for j, (lig, color) in enumerate(
                                    zip(ligand_list, fit_plot_colors)
                                ):
                                    # use is_gp_model=True to get data for all concentrations:
                                    stan_data = self.bs_frame_stan_data(
                                        row, initial=initial, is_gp_model=True
                                    )
                                    ...
                                    if len(antibiotic_conc_list) == 2:
                                        ...
                                        y = np.array(
                                            st_y_0
                                            + list(stan_data[f"y_{j+1}_{tet_str}_tet"])
                                        )
                                        s = np.array(
                                            st_y_0_err
                                            + list(
                ...
                if initial == plot_initials[0]:
                    ...
                    fitness_units = (
                        "log(10)/plate" if fitness_scale_factor == 1 else "1/h"
                    )
            ...
            if show_fits:
                if old_style_plots:
                    ...
                    tet_level_list = (
                        ["high"] if len(antibiotic_conc_list) == 2 else ["low", "high"]
                    )
                    for tet, marker in zip(tet_level_list, ["<", ">"]):
                        for lig, color in zip(ligand_list, fit_plot_colors):
                            ...
                            x = df[lig]
                            x_fit = np.logspace(
                                np.log10(linthresh / 10), np.log10(2 * max(x))
                            )
                            x_fit = np.insert(x_fit, 0, 0)
                            ...
                            if self.plasmid == "pVER":
                                params_list = [
                                    ...
                                    f"log_ec50_{lig}",
                                    f"sensor_n_{lig}",
                                    f"low_fitness_{tet}_tet",
                                    f"mid_g_{tet}_tet",
                                    f"fitness_n_{tet}_tet",
                                ]
                            elif self.plasmid == "pRamR":
                                params_list = [
                                    "log_g0",
                            ...
                            ylimr = axr.get_ylim()
                            ylimr = (
                                min(ylimr[0], min(y_fit) - 0.2),
                                max(ylimr[1], max(y_fit) + 0.2),
                            )
                            axr.set_ylim(ylimr)
                ...
                if old_style_plots:
                    ...
                    for i in range(1, 3):
                        axr.fill_between(
                            ...
                            stan_f[2 + i],
                            alpha=0.3,
                            color=fit_plot_colors[2],
                        )
                        if i == 3:
                            ylimr = axr.get_ylim()
                            ylimr = (
                                min(ylimr[0], min(stan_f[2 - i]) - 0.2),
                                max(ylimr[1], max(stan_f[2 + i]) + 0.2),
                            )
                            axr.set_ylim(ylim)
                        axg.fill_between(
                            x,
    ...
    def calibrate_fitness_difference_params(
        self,
        ...
        if plasmid == "pVER":
            ...
            def init_fitness_fit(y_data):
                ...
                sig = np.random.normal(1, 0.2) * 0.1
                return dict(
                    low_level=low, IC_50=mid, hill_n=n, sigma=sig, high_level=high
                )
        elif plasmid == "pCymR":
            stan_model_file = "Hill equation fit-zero high.stan"
        ...
        if len(plot_antibiotic_list) == 1:
            ...
            plt.rcParams["figure.figsize"] = [
                fig_size[0],
                fig_size[1] * len(plot_antibiotic_list),
            ]
            fig, axs = plt.subplots(len(plot_antibiotic_list), 1, layout="tight")

        if show_old_fit:
            if plasmid == "Align-TF":
                if (type(self.fit_fitness_difference_params) is dict) and (
                    spike_in_initial in self.fit_fitness_difference_params
                ):
                    params = self.fit_fitness_difference_params[spike_in_initial]
                    plot_fit_params = [
                        params[tet]["popt"] + params[tet]["perr"]
                        for tet in plot_antibiotic_list
                    ]
                else:
        ...
        for ax, tet, old_fit_params in zip(axs, plot_antibiotic_list, plot_fit_params):
            ...
            for RS_name in RS_list:
                for lig in ligand_plot_list:
                    ...
                    if len(df) == 1:
                        ...
                        hill_params = [
                            10 ** cytom_row[x]
                            for x in ["log_g0", "log_ginf", "log_ec50"]
                        ] + [cytom_row["n"]]
                            ...
                            if len(HiSeq_df) > 0:
                                ...
                                for ind, HiSeq_row in HiSeq_df.iterrows():
                                    ...
                                    if plasmid == "Align-TF":
                                        ...
                                        plot_df_align = plot_df_align[
                                            plot_df_align.antibiotic_conc == tet
                                        ]
                                        ...
                                        for lig_ref in lig_conc:
                                            df_ref = plot_df
                                            df_ref = df_ref[
                                                df_ref.transcription_factor == tf
                                            ]
                                        ...
                                        y = np.array(
                                            [
                                                HiSeq_row[
                                                    f"fitness_S{s}_{spike_in_initial}"
                                                ]
                                                for s in samples
                                            ]
                                        )
                                        y_err = np.array(
                                            [
                                                HiSeq_row[
                                                    f"fitness_S{s}_err_{spike_in_initial}"
                                                ]
                                                for s in samples
                                            ]
                                        )
                                        ...
                                        for lig_ref in lig_conc:
                                            if lig_ref == 0:
                                                g = cytom_row["log_g0"]
                                                gerr = np.sqrt(
                                                    cytom_row["log_g0_err"] ** 2
                                                    + min_xerr**2
                                                )
                                            else:
                                                g = cytom_row[
                                                    f"log_g_{int(lig_ref)}_{lig}"
                                                ]
                                                gerr = np.sqrt(
                                                    cytom_row[
                                                        f"log_g_{int(lig_ref)}_{lig}_err"
                                                    ]
                                                    ** 2
                                                    + min_xerr**2
                                                )

                                            gerr = fitness.log_plot_errorbars(
                                                log_mu=g, log_sig=gerr
                                            )
                                            g = 10**g
                                    ...
                                    elif (len(ligand_plot_list) > 1) or (
                                        plasmid == "pCymR"
                                    ):
                                        # For RamR and CymR
                                        lig_conc = np.array(
                                        ...
                                        if tet == plot_antibiotic_list[0]:
                                            ...
                                            samples = np.array(
                                                stan_data[f"samp_{i+1}_high_tet"]
                                            )
                                            y = np.array(stan_data[f"y_{i+1}_high_tet"])
                                            y_err = np.array(
                                                stan_data[f"y_{i+1}_high_tet_err"]
                                            )
                                    ...
                                    if sel:
                                        ...
                                        identifier_list += [
                                            f"{RS_name} at x = {p:.2e}" for p in x
                                        ]
                                        if color_by_ligand_conc is not None:
                                        ...
                                        if return_resid_table:
                                            ...
                                            resid_frame_early_fitness += [
                                                HiSeq_row[
                                                    f"fitness_S{smp}_ea.{spike_in_initial}"
                                                ]
                                                for smp in samples
                                            ]

                                    include_data_in_plot = show_exclude_data or (
                                        not sel
                                    )
                                    if (include_data_in_plot) and (
                                        color_by_ligand_conc is None
                                    ):
                                        ax.errorbar(
                                            x,
                                            y,
            ...
            if return_fit_data:
                df_ret = pd.DataFrame(
                    {"x": x_fit_list, "y": y_fit_list, "yerr": y_err_list}
                )
                fit_data_ret = []
            ...
            if color_by_ligand_conc is not None:
                ...
                for c in concentrations:
                    ...
                    ax.errorbar(
                        ...
                        fmt="o",
                        ms=ms,
                        label=f"[{color_by_ligand_conc}] = {c}",
                        alpha=alpha,
                    )
            ...
            if show_old_fit and (old_fit_params is not None):
                x_plot_fit = np.logspace(
                    np.log10((min(x_fit_list[x_fit_list > 0])) / 3),
                    np.log10(1.2 * max(x_fit_list)),
                )
                if 0 in x_fit_list:
            ...
            if run_stan_fit:
                ...
                if turn_off_cmdstanpy_logger:
                    ...
                    cmdstanpy_logger.disabled = True
                stan_data = dict(
                    x=x_fit_list, y=y_fit_list, y_err=y_err_list, N=len(x_fit_list)
                )
                if robust_error_model:
                ...
                for run_num in range(num_stan_re_runs):
                    if (
                        (run_num == 0)
                        or (len(drop_list) != len(old_drop_list))
                        or (not np.all(old_drop_list == drop_list))
                    ):
                        ...
                        stan_popt = [
                            np.mean(stan_fit.stan_variable(p)) for p in key_params
                        ]
                        stan_perr = [
                            np.std(stan_fit.stan_variable(p)) for p in key_params
                        ]
                ...
                x_plot_fit = np.logspace(
                    np.log10((min(x_fit_list[x_fit_list > 0])) / 1.5),
                    np.log10(1.2 * max(x_fit_list)),
                )
                if 0 in x_fit_list:
                    x_plot_fit = np.array([0] + list(x_plot_fit))
                ...
                ax.plot(
                    x_plot_fit, y_plot_fit, "--k", zorder=200, label="calibration fit"
                )
                if len(drop_list) > 0:
                    ax.plot(
                        dropped_x_list,
        ...
        if run_stan_fit and save_fitness_difference_params:
            if plasmid == "Align-TF":
                if type(self.fit_fitness_difference_params) is dict:
                    self.fit_fitness_difference_params[spike_in_initial] = (
                        stan_params_to_save
                    )
                else:
                    self.fit_fitness_difference_params = {
                        spike_in_initial: stan_params_to_save
                    }
            else:
    ...
    def set_ramr_fitness_correction(
        ...
        ramr_model = GradientBoostingRegressor(
            loss="squared_error", min_samples_split=min_samples_split
        )
    ...
    def plot_chimera_plot(
        ...
        axs.plot(
            x, y, "o", ms=7, alpha=alpha, label="Possible Chimeras, Total Counts ÷ 96"
        )
    ...
    def plot_hill_params(
        ...
        if everything_color is None:
            everything_color = fitness.gray_out(
                "xkcd:tea green", s_factor=0.7, v_factor=0.8
            )
        ...
        for input_frame, c, lab in zip(input_frames, in_colors, in_labels):
            for ax, name, x_param, x_err_label in zip(
                axs, param_names, x_param_list, x_err_label_list
            ):
                ...
                if error_bars:
                    ...
                    ax.plot(x, y, "o", ms=4, color=c, label=lab, alpha=in_alpha)
        ...
        if "log_g0" in self.barcode_frame.columns.values:
            plot_frame = plot_frame[
                plot_frame[f"log_ginf{lig_str}_err"] < log_ginf_err_cutoff
            ]
        else:
            plot_frame = plot_frame[
                plot_frame["log_high_level error"] < log_ginf_err_cutoff
            ]

        for ax, name, x_param, y_label, x_label in zip(
            axs, param_names, x_param_list, y_label_list, x_label_list
        ):
    ...
    def save_as_pickle(
        ...
        if file_exists and (not overwrite):
            ...
            t = t.replace(":", ".")
            t = t[:16]
            old_pickle_file = pickle_file.replace(
    ...
    def cleaned_frame(
        ...
        if "pacbio_KAN_mutations" in frame.columns:
            ...
            for reg in exclude_mut_regions:
                frame = frame[
                    (~frame[f"{reg}_confident_seq"]) | (frame[f"{reg}_mutations"] == 0)
                ]
    ...
    def plot_hill_param_density_scatter(
        ...
        for zip_tuple in zip(axs, x_params, log_params, x_axs_labels, y_axs_labels):
            ...
            hist_ret = fitness.density_scatter_plot(
                params_x,
                params_y,
                ax=ax,
                sort=True,
    ...
    def bs_frame_stan_data(
        ...
        if self.plasmid == "Align-TF":
            ...
            sample_map = sample_map[sample_map.growth_plate == 2].sort_values(
                by=["antibiotic_conc"]
            )
            ...
            if len(ligand_concentrations) != 2:
                raise ValueError(
                    f"Unexpected number of ligand concentrations for transcription_factor {tf}"
                )
            ...
            for lig_conc in ligand_concentrations:
                ...
                ref_sample = sample_map_lig[sample_map_lig.antibiotic_conc == 0][
                    "sample_id"
                ]
                if len(ref_sample) == 1:
                    ...
                    raise ValueError(
                        f"Unexpected number of samples with zero antibiotic: len(ref_sample): {len(ref_sample)}"
                    )

                y = np.array(
                    [
                        st_row[f"fitness_S{s}_{init}"]
                        for s, init in zip(samples, init_list)
                    ]
                )
                y_err = np.array(
                    [
                        st_row[f"fitness_S{s}_err_{init}"]
                        for s, init in zip(samples, init_list)
                    ]
                )

                y_ref = np.array(
                    [st_row[f"fitness_S{ref_sample}_{init}"] for init in init_list]
                )
                y_ref_err = np.array(
                    [st_row[f"fitness_S{ref_sample}_err_{init}"] for init in init_list]
                )
            ...
            low_fitness_mu = np.array(
                [
                    fit_fitness_difference_params[init][c]["popt"][0]
                    for c, init in zip(anti_conc_list, init_list)
                ]
            )
            ...
            mid_g_mu = np.array(
                [
                    fit_fitness_difference_params[init][c]["popt"][1]
                    for c, init in zip(anti_conc_list, init_list)
                ]
            )
            stan_data["mid_g_mu"] = mid_g_mu

            fitness_n_mu = np.array(
                [
                    fit_fitness_difference_params[init][c]["popt"][2]
                    for c, init in zip(anti_conc_list, init_list)
                ]
            )
            stan_data["fitness_n_mu"] = fitness_n_mu

            low_fitness_std = np.array(
                [
                    fit_fitness_difference_params[init][c]["perr"][0]
                    for c, init in zip(anti_conc_list, init_list)
                ]
            )
            ...
            mid_g_std = np.array(
                [
                    fit_fitness_difference_params[init][c]["perr"][1]
                    for c, init in zip(anti_conc_list, init_list)
                ]
            )
            stan_data["mid_g_std"] = mid_g_std

            fitness_n_std = np.array(
                [
                    fit_fitness_difference_params[init][c]["perr"][2]
                    for c, init in zip(anti_conc_list, init_list)
                ]
            )
            stan_data["fitness_n_std"] = fitness_n_std

            log_g_min, log_g_max, log_g_prior_scale, wild_type_ginf = (
                fitness.log_g_limits(plasmid=self.plasmid)
            )
        ...
        ramr_fitness_correction_params = getattr(
            self, "ramr_fitness_correction_params", None
        )
