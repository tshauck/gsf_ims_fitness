...
class ODFitnessFrame:
    def __init__(self, notebook_dir, experiment=None, wavelength="600", auto_save=True):
        ...
        print(
            f"Loading plate reader data and calculating fitness for experiment: {self.experiment}"
        )
        ...
        if len(worklist_files) != 5:
            print(
                f"Warning! Unexpected number of worklist .csv files. Expected 5 files; found {len(worklist_files)} files"
            )
        ...
        if file_warning:
            print(
                f"Warning! Experiment name, {self.experiment}, not a substring of the worklist filenames."
            )
            print(worklist_files)
        ...
        if len(gen5_files) != 5:
            print(
                f"Warning! Unexpecte number of plate reader output .txt files. Expected 5 files; found {len(gen5_files)} files"
            )
            print(gen5_files)
        ...
        for file in gen5_files:
            ...
            nrows = (
                read_line_numbers[read_number_to_use]
                - read_line_numbers[read_number_to_use - 1]
                - 4
            )
            gen5_frame = pd.read_csv(
                file,
                skiprows=skiprows,
                nrows=nrows,
        ...
        for i, plate in enumerate(sample_frames):
            if i > 0:
                for x_l, y_l, s_l, density, dens_err in zip(
                    x_lists,
                    y_lists,
                    sig_lists,
                    plate["end_density"],
                    plate["end_density_err"],
                ):
                    if density > 0:
                        y_l.append(density)
                        x_l.append(i + 1)
        ...
        fitness_frame["inducerConcentration"] = sample_frames[-1][
        ...
        for index, row in fitness_frame.iterrows():
            ...
            popt, pcov = curve_fit(
                fitness.line_funct, x_fit, np.log(y_fit), bounds=bounds, sigma=sigma
            )
        ...
        for index, row in fitness_frame.iterrows():
            if row["tet_concentration"] == 0:
                ...
                slope_0 = no_tet_frame[no_tet_frame["inducerConcentration"] == indConc][
                    "fitness"
                ].mean()

                def fit_funct(xp, mp, bp):
                    return fitness.bi_linear_funct(
                        xp - 2, mp, bp, m1=slope_0, alpha=np.log(5)
                    )
    ...
    def plot_cell_density(
        ...
        axes_array[4].set_ylabel(
            "Cell Density (OD$\mathregular{_{600}}$)", size=20, labelpad=10
        )
        if save_plots:
            pdf.savefig()
        if not show_plots:
    ...
    def plot_fitness_curves(
        ...
        for j, conc in enumerate(Tet_concentrations):
            ...
            for c, plas in zip(plot_colors, plasmids):
                ...
                axs.errorbar(
                    x, y, yerr=y_err, fmt=marker, label=label, markersize=size, color=c
                )
